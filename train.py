...
from nets.unet import Cascade_Unet
...
if __name__ == "__main__":
    ...
    #   Freeze_Train    是否进行冻结训练默认先冻结主干训练后解冻训练。
    Freeze_Train        = False
    ...
    Init_lr             = 1e-4
    Min_lr              = Init_lr * 0.01
    #   optimizer_type  使用到的优化器种类，可选的有adam、sgd，当使用Adam优化器时建议设置  Init_lr=1e-4，当使用SGD优化器时建议设置   Init_lr=1e-2
    ...
    model = Cascade_Unet(num_classes=num_classes, pretrained=pretrained, backbone=backbone).train()
